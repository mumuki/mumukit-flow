...
describe 'flow integration' do
  ...
  let(:flow) { build_flow guide }

  it "1: normal, 2: normal, 3: easy" do
    expect(flow.current_number).to be 1

    4.times { flow.submit! :failed }
    flow.submit! :passed
    expect(flow.current_number).to eq 2

    4.times { flow.submit! :failed }
    flow.submit! :passed
    expect(flow.current_number).to eq 3

    flow.submit! :passed
    expect(flow.current_number).to be nil
  end
  ...
  pending "1: easy, 2: bypassed, 3: easy" do
    ...
    flow.submit! :passed
    expect(flow.current_number).to be 3
  ...
  it "1: hard, 2: easy, 3: easy, 1: easy" do
    ...
    flow.submit! :passed
    expect(flow.current_number).to be 2
